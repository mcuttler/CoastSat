...
#%% plot transects over shoreline image
fig = plt.figure()
plt.plot(output['shorelines'][0][:,0],output['shorelines'][0][:,1])
for i,trans in enumerate(transects):
    plt.plot(transects[trans][0,0],transects[trans][0,1],'b.')
    plt.plot(transects[trans][1,0],transects[trans][1,1],'r.')
plt.axis('equal')

#%% tide correct data 
#input tide data is in local time (Australian West Coast, UTC +8 hrs), but code below converts to UTC
tide_path ='C:\\Users\\00084142\\The University of Western Australia\\Andrew Pomeroy - Mauritius_Shorelines\\Data\\FES_Tide_Data'
tide_file = os.path.join(tide_path, 'Tides_for_CoastSat.txt')

tide, output_corrected = SDS_tools.process_tide_data(tide_file, output,settings)    

settings['zref'] = 0 #height datum to correct to
settings['beach_slope'] = -0.1 #beach slope
cross_distance_corrected = SDS_tools.tide_correct(cross_distance,tide,settings)
    
#%% Export output and cross_distance to CSV
import pandas as pd
#full filepath and name for CSV
csv_path1 = os.path.join(filepath,sitename + '_output.csv')
csv_path2 = os.path.join(filepath,sitename + '_cross_distance.csv')
